...
import "./styles/CountryCodeDropdown.css";
...
export const countries = [
  ...
  { code: "PN", name: "Pitcairn Islands", dial: "870" },
  { code: "PL", name: "Poland", dial: "48" },
  { code: "PT", name: "Portugal", dial: "351" },
  { code: "PR", name: "Puerto Rico", dial: "1787" },
  { code: "QA", name: "Qatar", dial: "974" },
  { code: "RE", name: "Reunion Island", dial: "262" },
  ...
  { code: "ST", name: "Sao Tome", dial: "239" },
...
export const DEFAULT_CODE = "IN"; // Default country code (India)
export const DEFAULT_DIAL = "91";
/* ---------------- Searchable Country Code Dropdown ---------------- */
const CountryCodeDropdown = ({
  value,       // country CODE string e.g. "IN", "US"
  onChange,    // (countryCode, countryObj) => void
  disabled = false,
  ...
  const selected = useMemo(() => {
    // Strip "+" if present to match against the 'dial' property
    const valStr = String(value || "").replace(/^\+/, "");
    return (
      countries.find((c) => c.dial === valStr || c.code === valStr) ||
      countries.find((c) => c.dial === DEFAULT_DIAL) ||
      countries.find((c) => c.code === DEFAULT_CODE) ||
      countries[0]
    );
  }, [value]);
  ...
  useEffect(() => {
    if (open) {
      ...
      setActiveIndex(0);
      setTimeout(() => {
        const el = containerRef.current?.querySelector(".dialpad-country-search-input");
        el?.focus();
      }, 0);
    }
  }, [open]);
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === "Enter") {
      e.preventDefault();
      const country = filtered[activeIndex] || filtered[0];
  ...
  return (
    <div
      ref={containerRef}
      className={`dialpad-country-select ${disabled ? "is-disabled" : ""} ${compact ? "compact" : ""} ${className}`}
      onKeyDown={handleKeyDown}
    >
      <button
        type="button"
        className={`dialpad-country-trigger ${error ? "has-error" : ""}`}
        onClick={() => !disabled && setOpen((v) => !v)}
        aria-haspopup="listbox"
        aria-expanded={open}
        disabled={disabled}
      >
        <div className="dialpad-country-trigger-content">
          <Icon name={selected.code.toLowerCase()} size={20} className="flag-icon" />
          <span className="dialpad-country-trigger-text">{selected.code} +{selected.dial}</span>
          {!compact && <Icon name="chevron-down" size={14} />}
        </div>
      </button>

      {open && (
        <div className="dialpad-country-popover" role="dialog">
          <div className="dialpad-country-search">
            <input
              className="dialpad-country-search-input"
          ...
          <div className="dialpad-country-list" ref={listRef} role="listbox" tabIndex={-1}>
            {filtered.length === 0 && (
              <div className="dialpad-country-empty">No matches</div>
            )}
            ...
            {filtered.map((country, idx) => (
              <div
                key={country.code}
                className={
                  "dialpad-country-option" + (idx === activeIndex ? " is-active" : "")
                }
                onClick={() => handleSelectCountry(country)}
                role="option"
                aria-selected={selected.code === country.code}
              >
                <Icon name={country.code.toLowerCase()} size={20} className="flag-icon" />
