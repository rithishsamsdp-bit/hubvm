...
const Conversation = () => {
  ...
  useEffect(() => {
    if (pendingNotificationPhone && conversations.length > 0 && !conversationLoading) {
      const phone = String(pendingNotificationPhone);
      const match = conversations.find(c => {
        const convPhone = String(c.c_conversationPhoneNo || "");
        return convPhone === phone || convPhone.endsWith(phone) || phone.endsWith(convPhone);
      });
      if (match) {
  ...
  useEffect(() => {
    if (conversations.length > 0 && !conversationLoading && selectedLead === null) {
      const storedId = sessionStorage.getItem(SIDEBAR_KEY);
      if (storedId) {
        const storedIdNum = isNaN(storedId) ? storedId : Number(storedId);
        const match = conversations.find(
          (c) => c.c_conversationId === storedIdNum || c.c_conversationId === storedId
        );
        if (match) {
          handleCardClick(match);
        } else {
  ...
  const handleCardClick = useCallback((card) => {
    ...
    if (isPredictive) {
      ...
      const predictiveIdToUse = activeCall?.predictiveLeadId || card.c_conversationDetails?.predictiveID;
      const campaignIdToUse = activeCall?.campaignId || card.c_conversationDetails?.callCampaignId || localStorage.getItem("CampaignId") || "0";
  ...
  const currentCall = (activeCalls || []).find(
    (c) =>
      ...
      (selectedData?.c_conversationPhoneNo &&
        c.dialnumber &&
        String(c.dialnumber) === String(selectedData.c_conversationPhoneNo).replace(/^\+/, ""))
  );
  ...
  return (
    <div className="conversation_container">
      <div className="conversation_wrapper">
        ...
        <section 
          className="conversation_right_sidebar" 
          style={!authPlan?.options?.conversation?.aidata ? { gridTemplateColumns: "1fr auto" } : {}}
        >
        ...
        >
          <div className="callback_remainder_modal_header_container">
            <p className="callback_remainder_modal_header">Callback Reminder</p>
            <Button variant="empty" onClick={handleCancel}>
              <Icon name="close" color="#0F172A" size={14} />
            </Button>
          </div>
          <form
            className="callback_remainder_modal_form"
            onSubmit={handleSubmit}
          >
            <label className="callback_remainder_form_label">Select Date</label>
            <DateTimeRangePicker
              type="single"
              showTime={true}
              ...
              format="YYYY-MM-DD"
            />
            <p>You will be notified 30 mins before the scheduled reminder</p>
            <div className="callback_remainder_modal_footer">
