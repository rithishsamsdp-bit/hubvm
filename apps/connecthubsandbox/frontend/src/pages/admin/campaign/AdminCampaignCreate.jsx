...
import "./styles/AdminCampaignCreate.css";
import {
  Input,
  Select,
  Button,
...
const AdminCampaignCreate = () => {
  ...
  const validateRules = (currentData, currentErrors = {}) => {
    ...
    if (currentData.dialerType === "PREDICTIVE") {
      const maxAttempts = Number(currentData.campaignRules.limits.maxattemptsper_day);
      const maxTotalAttempts = Number(currentData.campaignRules.limits.maxtotalattempts);
      ...
      if (currentData.campaignRules.retryrules?.NO_ANSWER?.enabled) {
        ...
        if (requiredIntervals > 0) {
          ...
          // If only 1 attempt, no intervals should be provided
          newErrors.no_answer_intervals = "Max Attempts is 1, so no retry intervals are needed (no retries today).";
        } else {
          delete newErrors.no_answer_intervals;
        }
      }
  ...
  const handleSave = async () => {
    ...
    if (payloadData.dialerType === "PREDICTIVE") {
      ...
      if (payloadData.campaignRules.retryrules?.NO_ANSWER) {
        payloadData.campaignRules.retryrules.NO_ANSWER.intervalsminutes = [];
      }
      if (payloadData.campaignRules.retryrules?.BUSY) {
        payloadData.campaignRules.retryrules.BUSY.intervalsminutes = [];
      }
    }

    await createCampaign(payloadData);
    navigate(-1);
  };

  // Helper for dynamic placeholder
  const getIntervalPlaceholder = () => {
    const maxAttempts = Number(formData.campaignRules.limits.maxattemptsper_day);
    const count = maxAttempts > 0 ? maxAttempts - 1 : 0;
    if (count <= 0) return "e.g. 20, 30";
    // Generate example: "10, 20, 30"
  ...
  return (
    <div className="admin_create_campaign">
      {/* Navbar */}
      ...
      {/* Form */}
      <div className="admin_create_campaign_content">
        <div className="admin_create_campaign_form_step">
          {/* Name */}
          <div className="admin_create_campaign_list_container_modal_form_group">
          ...
          {formData.dialerType === "PREDICTIVE" && (
            <div className="admin_create_campaign_campaign_rules">
        ...
        {/* Buttons */}
        <div className="admin_create_campaign_list_form_footer">
