  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-background p-4 sm:p-6 md:p-8">
      <Card className="w-full max-w-[360px] shadow-2xl border-border bg-card">
        <CardHeader className="space-y-3 flex flex-col items-center pt-5 pb-2">
          ...
          <div className="text-center space-y-1">
            ...
            <CardDescription className="text-muted-foreground text-xs sm:text-sm">
              ...
              <span className="font-bold text-foreground">{maskEmail(loginData.m_memberMailId)}</span>
            </CardDescription>
          </div>
        </CardHeader>
        ...
        <CardContent className="px-5 sm:px-6 pb-4">
          {isLoggingIn ? (
            <div className="flex flex-col items-center justify-center py-10 space-y-4">
              <Loader />
              <p className="text-sm font-medium text-muted-foreground animate-pulse">Verifying...</p>
            </div>
          ) : (
          <Form {...form}>
                      ...
                      <div className="flex justify-between gap-2 sm:gap-3">
                        {[0, 1, 2, 3, 4, 5].map((index) => (
                          <Input
                            ...
                            type="text"
                            maxLength="1"
                            className="w-8 h-10 sm:w-10 sm:h-12 text-center text-lg font-black bg-slate-50 border-slate-200 focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all rounded-xl"
        ...
        <CardFooter className="flex flex-col space-y-4 px-5 sm:px-6 pb-6 text-center text-sm">
          <div className="text-muted-foreground">
            {timer > 0 ? (
              <p>
                Resend code in <span className="font-black text-primary tabular-nums">{timer}s</span>
              </p>
            ) : (
              <p>
      ...
      <p className="mt-4 text-center text-[10px] text-muted-foreground uppercase tracking-widest">
        &copy; {new Date().getFullYear()} Pulse Communications. All rights reserved.
      </p>
    </div>
  );
};
