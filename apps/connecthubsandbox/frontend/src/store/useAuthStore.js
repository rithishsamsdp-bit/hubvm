...
export const useAuthStore = create((set, get) => ({
  ...
  fetchNotifications: async (extensionNo, type, limit = 20, offset = 0) => {
    ...
    try {
      ...
      if (isCallbackType) {
        const callbackNotifications = items.filter(n => n.action !== 'MISSEDCALL');
        const normalizedCallbacks = callbackNotifications.map(n => ({
          ...n,
          notificationType: n.notificationType || n.action,
          isRead: n.notificationStatus === "READ",
          title: n.title ?? (
            (n.notificationType === "CALLBACK" || n.action === "CALLBACK")
              ? "Call back Reminder"
              : (n.notificationType === "INCOMINGSMS" || n.action === "INCOMINGSMS")
                ? "Incoming SMS"
        ...
        set((state) => ({
          notificationData: offset === 0
            ? normalizedCallbacks
            : [...state.notificationData, ...normalizedCallbacks]
        }));
      }
  ...
  getMenus: () => {
    ...
    if (authRole === "SUPERADMIN") {
      set({ menus: superAdminMenu });
      return;
    }

    if (authRole === "ADMIN") {
      const filtered = admin.filter(
        (item) => menuAccess?.[item.id] === true || item.id === "contactbook"
      );
    ...
    if (authRole === "USER") {
      const filtered = userMenu.filter(
        (item) => menuAccess?.[item.id] === true || item.id === "dashboard"
      );
      set({ menus: filtered });
      return;
    }
