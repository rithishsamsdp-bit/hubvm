...
# KAFKA_HOST="13.126.75.6"
# KAFKA_PORT=30092
# _bootstrap_servers = "13.126.75.6:30092"
...
async def send_message(topic: str, key: str | None, value: dict) -> bool:
    await create_topic_if_not_exists(topic)
    producer = await get_producer()
    key_bytes = key.encode("utf-8") if key else None
    value_bytes = json.dumps(value).encode("utf-8")
    print(f"value1 : {value_bytes}")
    
    try:
        print(f"value2 : {value_bytes}")

        status=await producer.send_and_wait(topic, key=key_bytes, value=value_bytes)
        logger.info(f"📤 Message sent to {topic} (key={key})")
        return (f"KafkaValue : {value_bytes},status : {status}")  # ✅ Return True if sent successfully
