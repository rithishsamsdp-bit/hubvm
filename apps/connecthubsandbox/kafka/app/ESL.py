...
if version_info >= (2,6,0):
    def swig_import_helper():
        ...
        fp = None
        spec = importlib.util.find_spec('_ESL', [dirname(__file__)])
        if spec is not None:
        ...
        try:
            ...
            raise ImportError("Could not import _ESL. Ensure it is built and accessible.") from e
        if fp is not None:
            try:
                _mod = importlib.util.find_spec('_ESL', fp, pathname, description)
            finally:
                fp.close()
            return _mod
