    ...
    try:
        ...
        if sip_req_uri_refer_norm == "pulseConference":
            ...
            if direction == "inbound":
                fscallerid = form_data.get("variable_sip_h_X-cliphonenumber")
            else:
                fscallerid = form_data.get("variable_sip_h_X-fscallerid")
            print("Transfer via refer")
            xml_content = await routing_service.Transfer(campaign_id, "onedb", Caller_Callee_ID_Number, To, media_ipv4, X_uniqueId, sip_req_uri_refer, caller_id_name, variable_sip_user_agent, variable_uuid, target_uuid, proxyIP, Caller_Callee_ID_Number, Caller_Destination_Number, refer_to, direction, Caller_Username, fscallerid)
        ...
        elif sip_req_uri_refer_norm == "pulseEmergency":
            # Emergency IVR playback or flow
            ivr_type = (form_data.get("variable_ivr_type") or form_data.get("variable_sip_h_X-ivr_type") or "").strip("'\"")
            ivr_audio_url = (form_data.get("variable_ivr_audio_url") or form_data.get("variable_sip_h_X-ivr_audio_url") or "").strip("'\"")
            ivr_flow_id = (form_data.get("variable_ivr_flow_id") or form_data.get("variable_sip_h_X-ivr_flow_id") or "").strip("'\"")
            account_id = form_data.get("variable_account_id")
            account_no = form_data.get("variable_account_no")
            
            logger.info(f"Routing: Emergency call to {To} | type={ivr_type} | account={account_id}")
            xml_content = await routing_service.Emergency(campaign_id, "onedb", To, ivr_type, ivr_audio_url, ivr_flow_id, account_id, account_no, caller_id_no)
            logger.info("Emergency call routing completed successfully")
        ...
        elif sip_req_uri_norm == "pulseOutboundPredictive":
            uuid = "out-" + form_data.get("variable_sip_h_X-uniqueId")
            accountPrefix = form_data.get("variable_accountPrefix")
            peerName = form_data.get("variable_peerName")
            customerNumber = form_data.get("variable_customerNumber")
            ...
            clinumberId = form_data.get("variable_clinumberId")
            call_limit = form_data.get("variable_call_limit")
            c_queuegroupId = form_data.get("variable_c_queuegroupId")
            leadId = form_data.get("variable_leadId")
            
            xml_content = await routing_service.outboundPredicitve("onedb", peerName, accountPrefix, customerNumber, media_ipv4, uuid, callerid, variable_sip_user_agent, variable_uuid, accountid, accountno, clinumberId, call_limit, c_queuegroupId, leadId)
...
async def freeswitch_directory(request: Request):
    ...
    logger.info(f"Directory request received: {form_data}")
    print(form_data)
...
<document type="freeswitch/xml">
    ...
    try:
        logger.info(f"Processing directory request for user: {username}, domain: {domain}")
        xml_content = await routing_service.Voicemail(domain, "onedb", username)
        logger.info(f"Directory XML generated successfully for user: {username}")
        return Response(content=xml_content, media_type="application/xml")
    
    except Exception as e:
        logger.error(f"Error processing directory request for user {username}, domain {domain}: {str(e)}", exc_info=True)
        error_xml = """<?xml version="1.0" encoding="UTF-8" standalone="no"?>
<document type="freeswitch/xml">
    <section name="result">
        <result status="not found">
