...
const peerSchema = z.object({
  ...
  prefix: z
    ...
    .regex(/^[0-9]+$/, "Prefix must be numeric"),
  pilotno: z.string().min(1, "Pilot Number is required"),
  outboundPrefix: z
    .string()
    .min(1, "Outbound Prefix is required")
    .regex(/^\+?[0-9]+$/, "Must be numeric (can start with +)"),
  inboundPrefix: z
    .string()
    .min(1, "Inbound Prefix is required")
    .regex(/^\+?[0-9]+$/, "Must be numeric (can start with +)"),
...
const SuperAdminCallingPeer = ({ externalModalOpen, onExternalModalClose }) => {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      if (editId) {
        await editPeer({ ...payload, id: editId });
      } else {
  ...
  const fieldsConfig = [
    ...
    { name: "prefix", label: "Prefix", placeholder: "Enter prefix" },
    {
      name: "pilotno",
      label: "Pilot Number",
  ...
  return (
    <>
      ...
      <Dialog open={peerModalOpen} onOpenChange={(v) => !v && handleCancel()}>
