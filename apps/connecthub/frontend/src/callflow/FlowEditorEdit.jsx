...
export default function FlowEditorEdit() {
    const { editCallflow, editCallflowLoading, viewCallflow, Nodes, Edges, viewCallFlowLoading } = useCallFlow();
    ...
    useEffect(() => {
        if (!name?.trim() || !editid?.trim()) {
            navigate("/admin-callflow");
        }
    }, [name, editid, navigate]);
    ...
    const idCounter = useRef(getInitialCounter(Nodes));
    const nextId = useCallback(() => String(idCounter.current++), []);

    const cascadeDeleteByParentId = useCallback((parentId) => {
        setNodes((curNodes) => {
            const childIds = new Set(
                curNodes.filter((n) => n.data?.fromNodeId === parentId).map((n) => n.id)
            );
            ...
            if (selectedNode && (selectedNode.id === parentId || childIds.has(selectedNode.id))) {
    ...
    const prepareInitialNodes = useCallback((rawNodes) => {
                ...
                data: {
                    ...
                    onDelete: () => {
                        if (node.type === "keypad" || node.type === "timeRule") {
                            ...
                            setEdges((cur) => cur.filter((e) => e.source !== node.id && e.target !== node.id));
                        }
                    },
                },
            };
        });
    }, [cascadeDeleteByParentId, selectedNode]);

    const prepareInitialEdges = useCallback((rawEdges) => {
        return rawEdges.map((edge) => {
            return {
                ...edge,
                data: {
                    ...(edge.data || {}),
                    onDelete: () => setEdges((cur) => cur.filter((e) => e.id !== edge.id)),
                },
            };
        });
    }, []);
    ...
    useEffect(() => {
        setNodes(prepareInitialNodes(Nodes));
        setEdges(prepareInitialEdges(Edges));
    }, [Nodes, Edges]);
            ...
            addEdge(
                {
                    ...
                    data: {
                        onDelete: () => setEdges((cur) => cur.filter((e) => e.id !== edgeId)),
                    },
                },
                eds
            )
    ...
    const handleDataChange = useCallback((newData, fullReplace = false) => {
        if (!selectedNode) return;
        setNodes((nds) =>
            nds.map((n) => (n.id === selectedNode.id ? { ...n, data: fullReplace ? { ...newData } : { ...n.data, ...newData } } : n))
        );
    ...
    const syncChildNode = useCallback(({ nodeId, label }) => {
        if (!nodeId) return;
        setNodes((nds) =>
    ...
    const removeChildNode = useCallback((nodeId) => {
        if (!nodeId) return;
                ...
                ...nds,
                {
                    ...
                    data: {
                        ...
                        onDelete: () => {
                            ...
                            setEdges((cur) => cur.filter((e) => e.source !== newId && e.target !== newId));
                        },
                    },
                },
            ]);
    ...
    const editFlow = useCallback(async () => {
        try {
            ...
            await editCallflow(editid, name, cleanedNodes, edges);
            navigate(-1);
        } catch (error) {
            console.error("Failed to Edit Callflow", error);
        }
    }, [nodes, edges, editid, name, navigate, editCallflow]);
    ...
    if (editCallflowLoading || viewCallFlowLoading) {
        return (
    ...
    return (
        <div className="FlowEditor">
            <FlowTopbar submit={editFlow} exit={exit} name={name} type="edit" />
