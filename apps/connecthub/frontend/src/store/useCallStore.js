...
export const callStore = create((set, get) => ({
  ...
  fetchConferenceData: async () => {
    try {
      ...
      const proxyId = authUser?.p_proxyId;

      const params = new URLSearchParams();
      params.append('conference_name', `${conferenceName}-${accountNo}`);
      if (proxyId) params.append('p_proxyId', proxyId);

      const response = await callingfeaturesaxios.get(
        `/callfeatures/conference/list?${params.toString()}`
      );
    ...
    try {
      ...
      if (Array.isArray(conferenceData) && conferenceData.length > 0) {
        ...
        if (participant?.conference_id) {
          try {
            const proxyId = authUser?.p_proxyId;
            const kickParams = proxyId ? `?p_proxyId=${proxyId}` : '';
            await conversationaxios.post(`/callfeatures/conference/${conferenceName}/kick/${participant.conference_id}/${accountNo}${kickParams}`);
  ...
  endConference: async () => {
    try {
      ...
      if (conferenceNameToEnd) {
        try {
          const proxyId = authUser?.p_proxyId;
          const kickAllParams = proxyId ? `?p_proxyId=${proxyId}` : '';
          await callingfeaturesaxios.post(
            `/callfeatures/conference/${conferenceNameToEnd}/kick-all/${accountNo}${kickAllParams}`,
          );
        } catch (apiError) {
          console.error(
