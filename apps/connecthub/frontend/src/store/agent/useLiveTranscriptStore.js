...
export const useLiveTranscriptStore = create((set, get) => ({
  ...
  startListening: async () => {
    ...
    // Immediately set to prevent double-calls during async setup
    set({ isListening: true, transcript: [], error: null });
    ...
    try {
      ...
      connection.on("message", (data) => {
        try {
          ...
          set((prev) => {
            ...
            const currentText = alt.transcript.trim();
            
            // Deduplicate: If same speaker and same text as last message, ignore
            if (lastMsg && lastMsg.speaker === speaker && lastMsg.text === currentText) {
              return prev;
            }
            ...
            return {
              transcript: [
                ...prev.transcript,
                { id: prev._idCounter + 1, speaker, text: currentText },
              ],
      ...
      // 7. Final check: did the user stop listening while we were setting up?
      if (!get().isListening) {
        console.log("[LiveTranscript] 🛑 Setup finished but isListening is false. Cleaning up.");
