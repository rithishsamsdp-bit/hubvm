...
class EventProcessor:
    ...
    def process_event(self, eventdata: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.info(f"accountId : {accountId}")
            logger.info(f"accountNo : {accountNo}")
            logger.info(f"clinumberId : {clinumberId}")
            ...
            logger.info(f"newInboundUser : {newInboundUser}")
    ...
    def insertMessageLog(self, accountId, accountNo, src_number, timestamp, text_data, agent_extension, agent_name, socket_users):
        ...
        if not checkSmsMode:
            logger.warning(f"⚠️ Failed to get SMS mode for {agent_extension}, defaulting to SOCKET")
            smsMode = 'SOCKET'
            mailId = None
        else:
