...
from models.dto import BillingCreate,BillingUpdate,BillingHistoryCreate,BillingHistoryListSelectModel,BillingConfigListSelectModel,BillingConfigListResponse,BillingHistoryListResponse, BalanceDeductListResponse, BalanceDeductListSelectModel
...
# calldeduct History API
@router.post(
    "/select_calldeduct_history",
    status_code=status.HTTP_200_OK,
    response_model=BalanceDeductListResponse
)
async def select_calldeduct_history_controller(
    request: BalanceDeductListSelectModel,
    tokenRequest: Request
):
    token = tokenRequest.cookies.get("accessToken")
    data = billing_service.decode(token)
    ...
    result = await billing_service.get_balance_deduct_list(
        accountEncryption=data.accountEncryption,
...
# biliingDashboard API
@router.get("/billingDashboard_details", status_code=200)
async def get_billingDashboard_details(tokenRequest: Request):
    ...
    result = await billing_service.get_dashboard(
        data.accountEncryption
    )
