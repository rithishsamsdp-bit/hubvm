...
# 🔹 Single Record DTO
class BalanceDeductList(BaseModel):
    account_id: int
    call_uuid: str
    b_from: Optional[str] = None

    b_CDR_billsec: Optional[int] = None
    b_pulsebill_overall_sec: Optional[str] = None
    b_pulseBilling: Optional[int] = None
...
# 🔹 Response DTO
class BalanceDeductListResponse(BaseModel):
    recordsTotal: int
    data: List[BalanceDeductList]


# 🔹 Filter / Query DTO
class BalanceDeductListSelectModel(BaseModel):
    limit: int = Field(1000, gt=0)
    offset: int = Field(0, ge=0)
