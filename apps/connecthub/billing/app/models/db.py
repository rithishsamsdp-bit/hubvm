from sqlalchemy import Integer, Float, String, Enum, DateTime, JSON, BigInteger, Text, CHAR, TIMESTAMP, Column, ForeignKey, text, Index, DECIMAL
...
class Accounts(Base):
    ...
    a_accountServiceRegion = Column("a_accountServiceRegion", Enum('Domestic','International','International-mid'), nullable=True)
...
class BillingConfig(Base):
    __tablename__ = 'p_billing_credit_config'

    b_billingId = Column(Integer, primary_key=True, index=True)
    b_billingAccountId = Column(Integer, nullable=True)
    b_billingAccountNO = Column(String(150), nullable=True)
    b_billingAccountName = Column(String(150), nullable=True)

    b_credit_balance = Column(DECIMAL(10,2), default=0.00)
    b_billing_status = Column(Enum('enable','disable'), default='enable')
    b_rate_per_min = Column(Float, default=1)
    b_billing_pulse = Column(Integer, default=30)
    b_billing_type = Column(String(100), default='prepaid')

    b_billingDescription = Column(Text, nullable=False)

    b_billingCreatedOn = Column(DateTime, server_default=func.now())
    b_billingUpdatedOn = Column(DateTime, server_default=func.now(), onupdate=func.now())
    b_billing_whocredit_AccountId = Column(Integer, nullable=True)


class BillingHistory(Base):
    __tablename__ = "p_billing_history"

    b_historyId = Column(Integer, primary_key=True, index=True)
    b_creditAccountId = Column(Integer)
    b_creditAccountName = Column(String(150))
    b_credit_balance = Column(DECIMAL(10,2))
    b_gst_percent = Column(DECIMAL(5,2), nullable=True, default=18.00)
    b_gst_amount = Column(DECIMAL(10,2), nullable=True)
    b_tds_percent=Column(DECIMAL(5,2), nullable=True)
    b_tds_amount=Column(DECIMAL(10,2), nullable=True)
    b_total_amount = Column(DECIMAL(10,2))
    b_creditDescription = Column(String(255), nullable=True)
    b_whoCredit = Column(String(100), nullable=True)
    b_transaction_id = Column(String(100), nullable=False, unique=True, index=True)
    b_paymentDoneBy = Column(String(150), nullable=True)
    b_creditCreatedOn = Column(DateTime, server_default=func.now())


class BalanceDeductHistory(Base):
    __tablename__ = "p_balance_deduct"

    id = Column(BigInteger, primary_key=True, index=True)

    account_id = Column(BigInteger, nullable=False, index=True)
    call_uuid = Column(String(100), nullable=False, index=True)

    b_from = Column(String(255))

    b_CDR_billsec = Column(Integer, default=0)
    b_pulsebill_overall_sec = Column(String(250))
    b_pulseBilling = Column(Integer, default=30)

    rate = Column(DECIMAL(10, 2), default=0.00)
    total_cost = Column(DECIMAL(12, 2), default=0.00)

    old_balance = Column(DECIMAL(14, 2), default=0.00)
    available_balance = Column(DECIMAL(14, 2), default=0.00)
    deduct_amount = Column(DECIMAL(12, 2), default=0.00)
