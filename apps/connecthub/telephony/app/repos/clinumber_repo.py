...
from models.db import CLINumbers, Accounts, Peers, CallFlows, Members, Locations, LocationGroups, API, SmsFlow, ExternalIntegrationAPIs, RelationalCLINumbersMembers, RelationalCLINumbersLocations, RelationalExternalIntegrationAPIsCLINumbersCallFlows
...
async def map(clinumbername: str, clinumberid: int, clinumbermapname: str, callflowid: Optional[int], callflowname: Optional[str], locationid: Optional[int], memberids: List[int], smsmembers: List[SMSMember], apiIntegration:str, apis: List[Dict[str, Any]], accountid: int, accountno: str, database: str):
    ...
    try:
        await session.execute(
            sql_update(CLINumbers).where(CLINumbers.c_clinumberId == clinumberid)
            .values(
        ...
        result = await session.execute(
            select(CLINumbers.c_clinumberCountryCode).where(CLINumbers.c_clinumberId == clinumberid)
        )
        ...
        await session.execute(
            Delete(RelationalCLINumbersLocations).where(
                RelationalCLINumbersLocations.r_clinumberId == clinumberid
            )
        )
        await session.execute(
        ...
        memberids_location = []
        if locationid:
            record = RelationalCLINumbersLocations(
                r_accountId = accountid,
                r_accountNo = accountno,
                r_clinumberId = clinumberid,
                r_locationId = locationid
            )
            session.add(record)
            result = await session.execute(
                select(LocationGroups.l_memberId).where(
                    LocationGroups.l_locationId == locationid
                )
            )
            memberids_location = result.scalars().all()
        memberids_merged = set(memberids) | set(memberids_location)
        if memberids_merged:
            await session.execute(
                RelationalCLINumbersMembers.__table__.insert(),
                [
                    {
                        "r_accountId": accountid,
                        "r_accountNo": accountno,
                    ...
                    for memberid in memberids_merged
                ]
            )
