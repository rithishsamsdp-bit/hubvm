...
class RelationalCLINumbersMembers(Base):
    ...
    r_memberMappingType = Column(Enum('INDIVIDUAL','LOCATION'), default="INDIVIDUAL", nullable=False)
...
class Members(Base):
    ...
    m_memberAccessStatus = Column('m_memberAccessStatus', Enum('Active','Inactive'), nullable=False, default='Active')
...
class Projects(Base):
    __tablename__ = "p_projects"

    p_projectId = Column(BigInteger, primary_key=True, autoincrement=True)
    p_accountId = Column(BigInteger, nullable=False)
    p_accountNo = Column(String(100), nullable=False)
    p_projectName = Column(String(255), nullable=False)
    p_createdOn = Column(TIMESTAMP, server_default=func.current_timestamp())

    project_groups = relationship("ProjectGroups", back_populates="project",cascade="all, delete-orphan")

class Locations(Base):
    __tablename__ = "p_locations"

    l_locationId = Column(BigInteger, primary_key=True, autoincrement=True)
    l_accountId = Column(BigInteger, nullable=False)
    l_accountNo = Column(String(100), nullable=False)
    l_locationName = Column(String(255), nullable=False)
    l_createdOn = Column(TIMESTAMP, server_default=func.current_timestamp())

    location_groups = relationship("LocationGroups", back_populates="location", cascade="all, delete-orphan")

class Departments(Base):
    __tablename__ = "p_departments"

    d_departmentId = Column(BigInteger, primary_key=True, autoincrement=True)
    d_accountId = Column(BigInteger, nullable=False)
    d_accountNo = Column(String(100), nullable=False)
    d_departmentName = Column(String(255), nullable=False)
    d_createdOn = Column(TIMESTAMP, server_default=func.current_timestamp())

    department_groups = relationship("DepartmentGroups", back_populates="department", cascade="all, delete-orphan")

class ProjectGroups(Base):
    __tablename__ = "p_projectgroups"

    p_projectId = Column(BigInteger, ForeignKey("p_projects.p_projectId", ondelete="CASCADE"), primary_key=True)
    p_memberId = Column(BigInteger, ForeignKey("p_members.m_memberId", ondelete="CASCADE"), primary_key=True)
    p_accountId = Column(BigInteger, nullable=False)
    p_accountNo = Column(String(100), nullable=False)
    p_createdOn = Column(TIMESTAMP, server_default=func.current_timestamp())

    project = relationship("Projects", back_populates="project_groups")

class LocationGroups(Base):
    __tablename__ = "p_locationgroups"

    l_locationId = Column(BigInteger, ForeignKey("p_locations.l_locationId", ondelete="CASCADE"), primary_key=True)
    l_memberId = Column(BigInteger, ForeignKey("p_members.m_memberId", ondelete="CASCADE"), primary_key=True)
    l_accountId = Column(BigInteger, nullable=False)
    l_accountNo = Column(String(100), nullable=False)
    l_createdOn = Column(TIMESTAMP, server_default=func.current_timestamp())

    location = relationship("Locations", back_populates="location_groups")

class DepartmentGroups(Base):
    __tablename__ = "p_departmentgroups"

    d_departmentId = Column(BigInteger, ForeignKey("p_departments.d_departmentId", ondelete="CASCADE"), primary_key=True)
    d_memberId = Column(BigInteger, ForeignKey("p_members.m_memberId", ondelete="CASCADE"), primary_key=True)
    d_accountId = Column(BigInteger, nullable=False)
    d_accountNo = Column(String(100), nullable=False)
    d_createdOn = Column(TIMESTAMP, server_default=func.current_timestamp())

    department = relationship("Departments", back_populates="department_groups")

class RelationalCLINumbersLocations(Base):
    __tablename__ = "r_clinumbers_locations"
    r_accountId = Column(BigInteger, nullable=True)
    r_accountNo = Column(String(100), nullable=True)
    r_locationId = Column(BigInteger, ForeignKey("p_locations.l_locationId", ondelete="CASCADE"), primary_key=True)
    r_clinumberId = Column(BigInteger, ForeignKey("p_clinumbers.c_clinumberId", ondelete="CASCADE"), primary_key=True)
    __table_args__ = (Index("IDX_Relational_CLINumbers_Locations_LocationId", "r_locationId"),)
