...
class LocationListResponse(BaseModel):
    ...
    class Location(BaseModel):
        l_locationId: str
        l_locationName: str
        members: List["LocationListResponse.Member"]
        class Config:
            from_attributes = True
    data: List[Location]
...
class LocationFetchRequest(BaseModel):
    limit: int = Field(1000, gt=0)
    offset: int = Field(0, ge=0)
    searchString: str = Field(default="")
    sortField: Literal["l_locationName", "l_locationId"] = "l_locationId"
    sortOrder: Literal["ASC", "DESC"] = "DESC"
    class Config:
        model_config = {'extra': 'forbid'}
...
class MemberAccessStatusUpdateModel(BaseModel):
    memberid: int
    memberaccessstatus: Optional[str] = None
...
class ProxyEmailTriggerRequest(BaseModel):
    proxyname: str
    sourceip: str
    domain: str