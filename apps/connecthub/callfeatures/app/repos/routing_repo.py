...
from models.db import CdrLogs, Campaigns, DidNumberGroup, RelationalDidnumbersGroups, PRelationalTableCampaignsDidNumberGroups,CLINumbers, Peers, Members, RelationalCLINumbersMembers,Accounts, MediaInstances
...
async def get_media_instances_by_proxy_id(proxy_id: int):
    """Fetch all media instance IPs for a given proxyId from the onedb database."""
    engine = asyncEngineFactory('onedb')
    async_session_maker = sessionmaker(engine, expire_on_commit=False, class_=AsyncSession)
    async with async_session_maker() as session:
        result = (await session.execute(
            select(MediaInstances.m_mediaPrivateIPAddress)
            .filter(MediaInstances.m_proxyId == proxy_id)
        )).scalars().all()
        return result
